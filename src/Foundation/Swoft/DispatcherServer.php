<?php
namespace IdoutvServiceOriented\Foundation\Swoft;
...
class DispatcherServer implements DispatcherInterface
{
    ...
    public function doDispatcher(...$params)
    {
        ...
        try {
            ...
            if ((bool) env('DEBUG') == true) {
                ini_set('display_errors', 1);
                $response = [];
                $response['message'] = '#Exception ' . $throwable->getMessage();
                $response['messageSep'] = str_repeat('-', strlen($response['message']));
                $response['line'] = "#File " . $throwable->getFile() . '(' . $throwable->getLine() . ')';
                $response['lineSep'] = str_repeat('-', strlen($response['line']));
                $response['trace'] = $throwable->getTraceAsString();
                $response = join("\n", $response); // Handle by ExceptionHandler
            } else {
