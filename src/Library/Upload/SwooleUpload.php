<?php
namespace IdoutvServiceOriented\Library\Upload;

use Swoft\Base\RequestContext;
use IdoutvServiceOriented\Library\Upload\Qiniu\QiniuDriver;
...
class SwooleUpload
{
 ...
 // 上传错误信息
    ...
    {
        if ('' === $files) {
            $files = RequestContext::getRequest()->getSwooleRequest()->files;
        }
        ...
        foreach ($files as $key => $file) {
            ...
            /* 调用回调函数检测文件是否存在 */
            $data = $this->callback ? call_user_func($this->callback, $file) : false;
    ...
    {
        $driver = $driver ?: ($this->driver ?: QiniuDriver::class);
