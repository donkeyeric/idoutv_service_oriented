<?php
namespace IdoutvServiceOriented\Library;
...
class Paginate
{
    ...
    public function parsePageLink()
    {
        if ($this->PageLink) {
            $component = parse_url($this->PageLink);
            
            if (isset($component['path'])) {
                $this->linkPath = $component['path'];
            }
            
            if (isset($component['query'])) {
                parse_str($component['query'], $this->linkArgs);
            }
        }
    }
    ...
    {
        $arg = [];
        foreach ($this->linkArgs as $key => $val) {
        ...
        if ($this->Static) {
            ...
            $url = $this->linkPath . "?" . http_build_query($arg);
    ...
    {
        $p = isset($this->linkArgs[$this->PageParam]) ? intval($this->linkArgs[$this->PageParam]) : 1;
        $p = $p < 1 ? 1 : $p;
